...
# Attendre que les conteneurs soient prets (iproute2/iputils installes)
echo ""
echo "Attente que les conteneurs soient prets..."
for container in pc1 pc2 pc3 router; do
    for i in $(seq 1 30); do
        if sudo docker exec "$container" ping -c 1 -W 1 127.0.0.1 &>/dev/null; then
            echo "  $container : pret"
            break
        fi
        if [ "$i" -eq 30 ]; then
            echo "  $container : TIMEOUT - non pret apres 30s"
        fi
        sleep 1
    done
done
...
run_test "PC1 -> PC2" "sudo docker exec pc1 ping -c 2 -W 5 $PC2_IP"

echo ""
echo "Test inter-VLAN (via Router) :"
run_test "PC1 -> PC3" "sudo docker exec pc1 ping -c 2 -W 5 $PC3_IP"
run_test "PC3 -> PC1" "sudo docker exec pc3 ping -c 2 -W 5 $PC1_IP"
